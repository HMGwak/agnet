...
export function localSidecarRoot() {
...
export function assertLocalCodexPath(candidatePath, sidecarRoot = localSidecarRoot()) {
  const resolvedCandidate = resolve(candidatePath);
  if (!isPathInsideDirectory(resolvedCandidate, sidecarRoot)) {
    throw new Error(
      `Resolved Codex runtime escaped the sidecar root: ${resolvedCandidate} is outside ${sidecarRoot}`
    );
  }
  return resolvedCandidate;
}

export function buildAllowlistEnv(runtimeHome, codexPath) {
  const env = {};
  ...
  if (codexPath) {
    ...
    env.PATH = inheritedPath ? `${codexDir}${delimiter}${inheritedPath}` : codexDir;
  }
  if (runtimeHome) {
...
function buildCodexOptions(payload, runtimeHome, codexPathOverride) {
  const options = {
    codexPathOverride,
    env: buildAllowlistEnv(runtimeHome, codexPathOverride),
  };
...
async function runCodexRun(runRecord, payload, runtimeHome, fallbackModel, resolveCodexPath) {
  const { Codex } = await import("@openai/codex-sdk");
  const localCodexPath = await resolveValidatedLocalCodexPath(resolveCodexPath);
...
async function runSession(runs, runId, payload, runtimeHome, fallbackModel, resolveCodexPath) {
...
export function createSidecarServer({
  ...
  host = "127.0.0.1",
  port = DEFAULT_PORT,
  resolveCodexPath = resolveLocalCodexPath,
}) {
  const runs = new Map();
  ...
  const server = createServer(async (req, res) => {
    if (req.method === "GET" && req.url === HEALTH_PATH) {
      try {
        const codexPath = await resolveValidatedLocalCodexPath(resolveCodexPath);
    ...
    if (req.method === "POST" && req.url === RUNS_PATH) {
      ...
      runSession(runs, runId, body, runtimeHome, DEFAULT_MODEL, resolveCodexPath);
...
async function resolveValidatedLocalCodexPath(resolveCodexPath = resolveLocalCodexPath) {
  const localCodexPath = assertLocalCodexPath(await resolveCodexPath());
  await access(localCodexPath);
  return localCodexPath;
}

export async function resolveLocalCodexPath() {
  const candidates = localCodexPathCandidates();
  for (const candidate of candidates) {
    const resolvedCandidate = assertLocalCodexPath(candidate);
    try {
  ...
  throw new Error(`local @openai/codex runtime was not found under ${localSidecarRoot()}`);
}
