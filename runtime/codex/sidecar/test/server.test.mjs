...
test("buildAllowlistEnv contains only allowlisted variables", () => {
  ...
  const codexPath = path.join(os.tmpdir(), "local-codex", process.platform === "win32" ? "codex.cmd" : "codex");
  const env = buildAllowlistEnv("/isolation", codexPath);
  assert.equal(env.PATH.split(path.delimiter)[0], path.dirname(codexPath));
...
test("assertLocalCodexPath rejects codex paths outside the sidecar root", () => {
  const sidecarRoot = fileURLToPath(new URL("../", import.meta.url));
  const externalPath = path.join(os.tmpdir(), "external-codex", process.platform === "win32" ? "codex.exe" : "codex");
  assert.throws(
    () => assertLocalCodexPath(externalPath, sidecarRoot),
    /outside/
  );
});

test("resolveLocalCodexPath returns a path within the sidecar root", async () => {
  const resolvedPath = await resolveLocalCodexPath();
  const sidecarRoot = fileURLToPath(new URL("../", import.meta.url));
  assert.equal(assertLocalCodexPath(resolvedPath, sidecarRoot), resolvedPath);
});
...
test("GET /health returns ready when auth cache and runtime are available", async () => {
  ...
  try {
    ...
    assert.equal(body.runtimeHome, dir);
    assert.equal(
      assertLocalCodexPath(body.codexPath, fileURLToPath(new URL("../", import.meta.url))),
      body.codexPath
    );
  } finally {
    await server.stop();
...
test("GET /health returns runtime error when resolver escapes the sidecar root", async () => {
  const { dir } = await makeFixtureAuth();
  const server = createSidecarServer({
    port: 0,
    runtimeHome: dir,
    resolveCodexPath: async () =>
      path.join(os.tmpdir(), "external-codex", process.platform === "win32" ? "codex.exe" : "codex"),
  });
  const port = await server.start();
  try {
    const response = await fetch(`http://127.0.0.1:${port}/health`);
    const body = await response.json();
    assert.equal(response.status, 500);
    assert.equal(body.status, "RUNTIME_NOT_FOUND");
    assert.match(body.detail, /outside/);
  } finally {
    await server.stop();
    await rm(dir, { recursive: true, force: true });
  }
